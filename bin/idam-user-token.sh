...
#curl ${CURL_OPTS} -XPOST "${IDAM_URI}/authorizationCode?username=ccd-importer@server.net&password=Password12&redirect_uri=${REDIRECT_URI}&client_id=${CLIENT_ID}" -d ""
...
#curl ${CURL_OPTS} -XPOST "${IDAM_URI}/authorizationCode?username=ccd-importer@server.net&password=Password12&redirect_uri=${REDIRECT_URI}&client_id=${CLIENT_ID}" -d "" | jq -r .code
